...
// Note: !! Your version of pg must support string->INT conversion (thus "1" = 1) and "" is a NULL value !!
...
class ajaxCRUD{
    ...
    function ajaxCRUD($item, $db_table, $db_table_pk, $ajaxcrud_root = "") {
        ...
        $this->loading_image_html = "<center><br /><br  /><img src=\'" . $this->ajaxcrud_root . "css/images/loading.gif\'><br /><br /></center>"; //changed via setLoadingImageHTML()
	...
	echo "<script type=\"text/javascript\" src=\"http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js\"></script>\n"; //per release 8, using jquery instead of protoculous
        echo "<script src=\"" . $this->ajaxcrud_root . "include/javascript_functions.js\" type=\"text/javascript\"></script>\n";
	...
	function doAction($action){
		...
		if ($action == 'add'){
            ...
            if ($_REQUEST['table'] == $this->db_table){
                ...
                //for getting datestamp of new row for pg's "NOW" to work
	...
	function showTable(){
        ...
        */
        ...
        if ($row_count == 0){
            $report_msg[] = "This table has no rows.";
        }
...
class paging{
	...
	function myRecordCount($query){
		$rs      			= pg_query($query) or die(pg_error()."<br>".$query);
		$rsCount 			= pg_num_rows($rs);
